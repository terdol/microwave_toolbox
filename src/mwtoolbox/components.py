...
from numpy.lib.scimath import (
    sqrt as csqrt,
)  # this works with negative real numbers unlike numpy.sqrt
...
def zo_eeff_wire_on_grounded_substrate(arg, defaultunits=None):
    ...
    eeff = ((er + 1.0) / 2.0) + (er - 1.0) / 2.0 * (
        (1.0 + 12.0 * h / 2.0 / d) ** (-0.5) + 0.04 * (1.0 - 2.0 * d / h) ** 2
    )
...
def ind_straight_flat_wire(arg, defaultunits=None):
    ...
    T = csqrt(((0.873011 + 0.00186128 * x) / (1.0 - 0.278381 * x + 0.127964 * x * x)))
    L = (
        0.002
        * l
...
def ind_air_core_coil(arg, defaultunits=None):
    ...
    argout = [ind, SRF]
    arg = arg + [
        prettystring(argout[i], defaultunits[len(arg) + i]) for i in range(len(argout))
    ]
    return arg
    ...
    L = (
        ...
        * scipy.integrate.quad(
            lambda x: np.log(
                ...
                / r
                * (
                    csqrt((d / 2.0 / np.sin(beta)) ** 2.0 - x**2.0)
                    + (h - d / 2.0 / np.tan(beta))
                )
            ),
            0,
...
def chebyshev_quarter_wave_impedance_transformer(arg, defaultunits=None):
    ...
    for i in range(3, N + 1):
        ...
        for j in range(2, N + 1):
            L[i - 1, j - 1] = (L[i - 2, j - 2] + L[i - 2, j]) * L[1, 1] - L[
                i - 3, j - 1
            ]
    if N % 2 == 0:
    ...
    for i in range(1, N + 1):
        z[i] = z[i-1] * np.exp(m[i - 1])

    thetam = (2.0 - 2.0 * (BW - 1.0) / (BW + 1.0)) * pi / 4.0
    print("s ", r)
    print("s ", N)
    print("s ", thetam)
    ...
    gamma = abs(
        (r - 1) / (r + 1)
        * scipy.special.chebyt(N - 1)((np.cos(pi / (fmin + fmax) * ftest) / np.cos(thetam)))
        / scipy.special.chebyt(N - 1)((1.0 / np.cos(thetam)))
    )
    arg.append(globsep2.join([prettystring(x, defaultunits[6]) for x in z]))
    arg.append(prettystring(20.0 * np.log10(gamma), defaultunits[7]))
    return arg
...
def binomial_quarter_wave_impedance_transformer(arg, defaultunits=None):
    ...
    for i in range(1, N + 1):
        a = (
            ...
            * scipy.misc.factorial(N)
            / scipy.misc.factorial(N - i + 1)
            / scipy.misc.factorial(i - 1)
        )
    ...
    BW = (
        ...
        / pi
        * arcnp.cos(
            0.5 * ((gamma / abs(2.0 ** (-N) * (r - 1) / (r + 1)))) ** ((1.0 / N))
        )
    ) * fcenter
...
def tee_attenuator_analysis(arg, defaultunits=None):
    ...
    Z1 = Zx + Rs
    S11 = 20.0 * np.log10((np.fabs(Z1 - Zo) / (Z1 + Zo)))
    K = (Rs + Zo) * Rp / (Rs + Zo + Rp) / Z1 * Zo / (Zo + Rs)
    Gamma = (Z1 - Zo) / (Z1 + Zo)
    S21 = 20.0 * np.log10(K * (1.0 + Gamma))
...
def pi_attenuator_synthesis(arg, defaultunits=None):
    ...
    print(Zo)
    K = 10.0 ** ((S21 / 20.0))
    R1 = Zo * (1 - K * K) / 2.0 / K  # Series resistor
...
def pi_attenuator_analysis(arg, defaultunits=None):
    ...
    Zt = Rp * Zo / (Rp + Zo) + Rs
    Z1 = Zt * Rp / (Zt + Rp)
    S11 = 20.0 * np.log10((abs(Z1 - Zo) / (Z1 + Zo)))
    K = Rp * Zo / (Rp + Zo) / Zt
    Gamma = (Z1 - Zo) / (Z1 + Zo)
    S21 = 20.0 * np.log10(K * (1 + Gamma))
    # power dissipated at first resistor
    P1 = 10.0 * np.log10((1 + Gamma) ** 2 * Zo / Rp)
    # power dissipated at last resistor
    P3 = 10.0 * np.log10((K * (1 + Gamma)) ** 2 * Zo / Rp)
    # power dissipated at middle resistor
    P2 = 10.0 * np.log10(((1 + Gamma) * (1 - K**2)) ** 2 * Zo / Rs)
    argout = [S11, S21, P1, P2, P3]
    arg = arg + [
        prettystring(argout[i], defaultunits[len(arg) + i]) for i in range(len(argout))
    ]
...
def bridged_tee_attenuator_analysis(arg, defaultunits=None):
    ...
    s = triangle_to_star_transformation(
        [prettystring(x) for x in [0, 0, 0, Rs, Zo, Zo]],
    ...
    temp = tee_attenuator_analysis(
        [prettystring(x) for x in [Zo, R1, R2]],
        ["ohm", "ohm", "ohm", "", "", "", "", ""],
    )
    s11 = temp[3]
    s21 = temp[4]
...
def symmetric_lange_coupler(arg, defaultunits=None):
    ...
    q = csqrt(C * C + (1.0 - C * C) * (n - 1) ** 2)
    a = csqrt(((1.0 - C) / (1.0 + C)))
    b = (n - 1) * (1.0 + q) / ((C + q) + (n - 1) * (1.0 - C))
    Zoo = Zo * a * b
    Zoe = Zoo * (q + C) / (n - 1) / (1.0 - C)
...
def optimum_mitered_arbitrary_angle_microstrip_bend(arg, defaultunits=None):
    ...
    b = [0, 30, 60, 90, 120]
    dd = np.meshgrid(a, b)
    z = np.array([[0, 0, 0], [12, 19, 7], [45, 41, 31], [75, 63, 56], [98, 92, 79]])
    ...
    if wh <= 2.0 and wh >= 0.0 and angle <= 120.0 and angle >= 0.0:
        x = griddata(
            list(zip(dd[0].flatten(), dd[1].flatten())),
            z.flatten(),
            (wh, angle),
            method="cubic",
        )
...
def interference_phase_amp_error(arg, defaultunits=None):
    ...
    arg.append(prettystring(20 * np.log10(((1.0 + x) / (1.0 - x))), ""))
...
def parallel_plate_cap(arg, defaultunits=None):
    ...
    corr = eps0 * w / np.pi * (np.log(2 * np.pi * l / h) + 1) + eps0 * l / np.pi * (
        np.log(2 * np.pi * w / h) + 1
    )
    ...
    arg.extend(
        [prettystring(cap, defaultunits[5]), prettystring(impedance, defaultunits[6])]
    )
    return arg
...
def circular_plate_cap(arg, defaultunits=None):
    ...
    impedance = 1.0 / (2.0 * np.pi * freq * cap)
    arg.extend(
        [prettystring(cap, defaultunits[4]), prettystring(impedance, defaultunits[5])]
    )
    return arg
...
def z_wg_te10(er, a, b, freq, formulation=1):
    ...
    if formulation == 1:  # Power-Voltage
        imp = csqrt(mu0 / eps0 / er) * k / beta * 2.0 * b / a
    elif formulation == 2:  # Voltage-Current
        imp = csqrt(mu0 / eps0 / er) * k / beta * pi * b / 2.0 / a
    elif formulation == 3:  # Power-Current
        imp = csqrt(mu0 / eps0 / er) * k / beta * pi**2 * b / 8.0 / a
...
def homogeneous_rect_waveguide_parameters_te(arg, defaultunits=None):
    ...
    skin_depth = csqrt((1.0 / (pi * freq * mu0 * sigma)))
    ...
    if mode == 0:  # TE mode
        if n == 0:
            ...
            alpha_c = (0.5 + b / a) * ((kc / k)) ** 2 + (
                b**2 * m**2 + a * b * n**2
            ) / (b**2 * m**2 + a**2 * n**2) * (1.0 - (kc / k) ** 2)
            alpha_c = 2 * Rs * alpha_c / b / eta / csqrt(1.0 - ((kc / k)) ** 2)
    elif mode == 1:  # TM mode
        alpha_c = (b**3 * m**2 + a**3 * n**2) / (a * b**2 * m**2 + a**3 * n**2)
        alpha_c = 2 * Rs * alpha_c / b / eta / csqrt(1.0 - ((kc / k)) ** 2)
    alpha_d = 0.5 * k * tand / csqrt(1.0 - ((kc / k)) ** 2)
...
def inductive_post_in_waveguide(arg, defaultunits=None):
    ...
    print(
        "Valid if frequency is between "
        + str(co / np.sqrt(er) / 2.0 / a)
        + " and "
        + str(co / np.sqrt(er) / a)
    )
    kc = pi / a
    k = 2 * pi * freq * csqrt(er) / co
    Zo = (
        csqrt(mu0 / eps0 / er) * k / np.sqrt(k * k - kc * kc) * 2.0 * b / a
        ...
        for cc in a:
            temp = np.append(
                ...
                sum(
                    [
                        np.sin(n * pi * x / cc) ** 2
                        * ((1.0 / csqrt(n**2 - (k * cc / pi) ** 2)) - (1.0 / n))
                        for n in range(2, 100)
                    ]
                ),
            )
    So = (
        np.log(4 * a / pi / d * np.sin(pi * x / a))
        - 2.0 * np.sin(pi * x / a) ** 2
        + 2.0 * temp
    )
    if isinstance(x, float) and isinstance(a, float):
    ...
    deltaX = (
        ...
        * (
            So
            - (k * d / 4.0) ** 2
            - (pi * d / 2.0 / a) ** 2 * ((So / np.tan(pi * x / a)) - S1) ** 2
        )
    )  # Xa-Xb/2.0
    Xa = (Xb / 2.0) + deltaX
    L = Xa / 2.0 / pi / freq / Zo
...
def inductive_window_in_waveguide(arg, defaultunits=None):
    ...
    def integrand(alpha):
        ...
        return (
            scipy.special.ellipk(alpha) - alpha1**2 * scipy.special.ellipe(alpha)
        ) / (scipy.special.ellipk(alpha1) - alpha**2 * scipy.special.ellipe(alpha1))
        ...
        for i in tt:
            alpha = append(
                ...
                scipy.optimize.brentq(
                    lambda x: integrand(x) - i, 0.001, 0.999, maxiter=2000
                ),
            )
    alpha1 = csqrt(1 - alpha**2.0)
    D2 = d / np.sqrt(2) * alpha1
    D2 = D2 * (
        (1.0 / (scipy.special.ellipe(alpha1) - alpha**2 * scipy.special.ellipk(alpha1)))
    )
    ...
    alpha1 = csqrt(1 - alpha**2.0)
    D1 = (
        csqrt(np.sqrt(alpha**2 * alpha1**2 / 3.0))
        * d
...
def evanescent_wg_equivalent(arg, defaultunits=None):
    ...
    gamma = 2.0 * pi / wavelength * csqrt(((wavelength / lcutoff)) ** 2.0 - 1.0)
    Xs1 = Xo * np.sinh(gamma * length)
    Xs2 = Xo * np.tanh(gamma * length / 2.0)
    Xp1 = Xo / np.tanh(gamma * length / 2.0)
    Xp2 = Xo / np.sinh(gamma * length)
    arg.append(prettystring(Xs1 / 2.0 / pi / frek, defaultunits[5]))
    arg.append(prettystring(Xp1 / 2.0 / pi / frek, defaultunits[6]))
    arg.append(prettystring(Xs2 / 2.0 / pi / frek, defaultunits[7]))
    arg.append(prettystring(Xp2 / 2.0 / pi / frek, defaultunits[8]))
...
def rect_wg_to_evanescent_rect_wg_step(a1, a2):
    ...
    n = (
        csqrt((a1 / a2))
        * (1.0 - ((a2 / a1)) ** 2.0)
        / (4.0 / pi * np.cos(pi / 2.0 * a2 / a1))
    )
    L = a1 * mu0 * np.tan(pi / 2.0 * a2 / a1) ** 2.0 / pi
...
def thermal_conductance_of_via_farm(arg, defaultunits):
    ...
    A = w * h - n * pi * ((d / 2.0)) ** 2
    st = (
        st * (sm * A / t / 2.0) / (st + sm * A / t / 2.0)
    )  # Alt ve ust toprak tabakalarını da kattık
    argout = [st, (1.0 / st)]
    arg = arg + [
        prettystring(argout[i], defaultunits[len(arg) + i]) for i in range(len(argout))
    ]
...
def exponential_taper_impedance_transformer(arg, defaultunits=None):
    ...
    max_gamma = (
        20.0 * np.log10(np.fabs(0.5 * np.log(r) * np.sin(2 * pi * Ls) / (2 * pi * Ls)))
    ).max()
...
def triangular_taper_impedance_transformer(arg, defaultunits=None):
    ...
    z_n2 = (
        np.exp(
            (2 * (np.arange(float(N)) / N) + 0.5 - 2.0 * (np.arange(float(N)) / N) ** 2)
            * np.log(r)
        )
        * Z0
    )
    ...
    Ls = np.linspace(L_lambda_min, L_lambda_max, 50)
    max_gamma = (
        20.0 * np.log10(np.fabs(0.5 * np.log(r) * ((np.sin(pi * Ls) / (pi * Ls))) ** 2))
    ).max()

    arg.append(
        globsep2.join(
            [prettystring(x, defaultunits[5]) for x in (list(z_n1) + list(z_n1) + [ZL])]
        )
    )
    arg.append(prettystring(max_gamma, defaultunits[6]))
    return arg
...
def patch_antenna_analysis(arg, defaultunits=None):
    ...
    p = (
        1
        + 0.1 * a2 * (k0 * w) ** 2
        + (a2**2 + 2 * a4) * (3 / 560) * (k0 * w) ** 4
        + c2 * (k0 * L) ** 2 / 5
        + a2 * c2 * (k0 * w) ** 2 * (k0 * L) ** 2 / 70
    )
    Qd = 1 / tand
    Qc = eta0 * k0 * h / 2 / Rs
...
def chebyshev_taper_impedance_transformer(arg, defaultunits=None):
    ...
    z_n = Z0 * np.exp(
        np.log(r) / (2 * pi) * (p + pi + 0.632 * np.sin(p) - 0.0653 * np.sin(3 * p))
    )
    ...
    max_gamma = (
        (
            20.0
            * np.log10(np.fabs(0.5 * np.log(r) * ((np.sin(pi * Ls) / (pi * Ls))) ** 2))
        )
    ).max()
...
def klopfenstein_taper_impedance_transformer(arg, defaultunits=None):
    ...
    def fi(ys, A):
        return np.array(
            [
                scipy.integrate.quad(
                    lambda x: (
                        scipy.special.iv(1, A * csqrt(1.0 - x * x))
                        / (A * csqrt(1.0 - x * x))
                    ),
                    0,
                    y,
    ...
    z_n = np.exp(
        0.5 * np.log(Z0 * ZL) + Gamma_0 / np.cosh(A) * (A * A * fi(2.0 * p - 1.0, A))
    )
    if ZL1 < Z01:
...
def absorptive_filter_equalizer(arg, defaultunits=None):
    ...
    a = (
        ...
        * k
        * np.sin(theta)
        / (np.sqrt(1.0 - k * k) * np.cos(theta) + 1.0j * np.sin(theta))
    )
    b = np.sqrt(1.0 - k * k) / (
        np.sqrt(1.0 - k * k) * np.cos(theta) + 1.0j * np.sin(theta)
    )
...
def parallelRC_to_seriesRC(Rp, Cp, freq):
    w = 2 * np.pi * freq
    Z = (Rp / (1j * w * Cp)) / (Rp + 1 / (1j * w * Cp))
    Rs = np.real(Z)
    Cs = - 1 / (w * np.imag(Z))
    return Rs, Cs

def seriesRC_to_parallelRC(Rs, Cs, freq):
    w = 2 * np.pi * freq
    Z = Rs + 1 / (1j * w * Cs)
    Y = 1. / Z
    Rp = 1. / np.real(Y)
...
def series_parallel_RC_analysis(arg, defaultunits=None):
    r"""Convert series RC to parallel RC at single frequency.

    Args:
        arg(list): First 3 arguments are inputs.

            1.  Frequency ; frequency
            2.  Series Resistance (Rs) ; impedance
            3.  Series Capacitance (Cs); capacitance
            4.  Parallel Resistance (Rp) ; impedance
            5.  Parallel Capacitance (Cp) ; capacitance
    ...
    arg = arg[:3]
    newargs = convert2pq(arg, defaultunits)
    freq, Rs, Cs = tuple(newargs)
    Rp, Cp = seriesRC_to_parallelRC(Rs, Cs, freq)
    argout = [Rp, Cp]
    arg = arg + [
        prettystring(argout[i], defaultunits[len(arg) + i]) for i in range(len(argout))
    ]
...
def series_parallel_RC_synthesis(arg, defaultunits=None):
    r"""Convert parallel RC to series RC at single frequency.
    ...
    arg = arg[:5]
    newargs = convert2pq(arg, defaultunits)
    freq, _, _, Rp, Cp = tuple(newargs)
    Rs, Cs = parallelRC_to_seriesRC(Rp, Cp, freq)
    argout = [Rs, Cs]
    arg[1] = prettystring(argout[0], defaultunits[1])
    arg[2] = prettystring(argout[1], defaultunits[2])
    return arg
...
def rect_cavity_res_freq(lengths, modenumbers, epsr, mur):
    """Calculates resonance frequency of a metallic cavity filled with
    dielectric.

    Args:
        lengths: Dimensions (in m)
    ...
    Returns:
        Frequency in Hz
    """
    freq = co / 2 / np.sqrt(epsr * mur) * np.sqrt(np.sum([(x/y)**2 for x, y in zip(modenumbers, lengths)]))
    return freq

def rect_cavity_res_freq_analysis(arg, defaultunits=None):
    r"""Calculates resonance frequencies of a dielectric filled rectangular cavity.
    ...
    Args:
        arg(list): First 5 arguments are inputs.
            ...
            13.  Mode-8 ; frequency
            14.  Mode-9 ; frequency
            15.  Mode-10 ; frequency
            16.  Mode-11 ; frequency
            17.  Mode-12 ; frequency
            18.  Mode-13 ; frequency
            19.  Mode-14 ; frequency
            20.  Mode-15 ; frequency
            21.  Mode-16 ; frequency
        defaultunits(list, optional): Default units for quantities in *arg* list. Default is [] which means SI units will be used if no unit is given in *arg*.
    ...
    if not defaultunits:
        defaultunits = [""] * 100
    arg = arg[:5]
    lengths = convert2pq([a for a in arg[:3] if a.strip()!=""], defaultunits[0])
    epsr, mur = tuple(convert2pq(arg[3:5], defaultunits[3:5]))
    modelist = itertools.combinations_with_replacement(list(range(1, 16 + 1)), len(lengths))
    mode_freqs_list = [(indices, rect_cavity_res_freq(lengths, indices, epsr, mur)) for indices in modelist]
    mode_freqs_list.sort(key=lambda x: x[1])
    # arg = arg + [prettystring(argout[i], defaultunits[len(arg) + i]) for i in range(len(argout))]
    # arg.append(globsep2.join([f"{k},{l},{m},{freq:e}" for k, l, m, freq in mode_freqs_list]))
    for indices, freq in mode_freqs_list:
        arg.append(f"({';'.join([str(x) for x in indices])})={prettystring(freq, defaultunits[5])}")
    return arg
...
if __name__ == "__main__":
    ...
    # print(
    #     optimum_mitered_arbitrary_angle_microstrip_bend(
    #         ["300", "127", "90deg", "3"], ["um", "um", "deg", "um"]
    #     )
    # )
    print(rect_cavity_res_freq_analysis(["200mil","300mil","400mil","1",1]))
