...
def generate_multiport_spfile( conf_filename, output_filename ):
    ...
    for i in range( len( target_indices ) ):
        newspfile.copy_data_from_spfile( target_indices[ i ][ 0 ], target_indices[ i ][ 1 ], source_indices[ i ][ 0 ], source_indices[ i ][ 1 ], spfiles[ spfilelist[ i ] ] )
...
class spfile:
    ...
    Attributes:
        data_format(str): Format of the data in the file if the data is read from a Touchstone file. Possible values are "DB", "MA", "RI".
        freq_unit(str):
    """
    ...
    def read_file( self, file_name, skiplines=0, only_port_number=False ):
        ...
        if self.file_data_format == "RI":
            ...
            lsonuc = [
              10**( ( datalar[ :b, 2*i + 1 ]/20.0 ) )*np.cos( datalar[ :b, 2*i + 2 ]*np.pi/180 ) + 10**( ( datalar[ :b, 2*i + 1 ]/20.0 ) )*np.sin( datalar[ :b, 2*i + 2 ]*np.pi/180 )*1j
              for i in range( ps**2 )
              ]
        else:
